...
function ($, ko, _) {
    ...
    ko.bindingHandlers.enter = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            var fnToInvoke = valueAccessor().execute;// assume this is a observable func(asyncCommand) ! // unwrap(valueAccessor());

            $(element).keyup(function (evt) {
                if(evt.keyCode == 13) {
                    if(_.isFunction(fnToInvoke)) {
                        fnToInvoke();
                    }
                }
            });
    ...
    ko.bindingHandlers.escape = {
        update: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            var command = valueAccessor(),
                valBinding = allBindingsAccessor().value;
